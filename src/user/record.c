...
void record_write_all(void)
{
    ...
    flash_direct_write_word(STEPPER_LOCATION_ADDRESS, stepper_location_set);
...
void record_read_all(void)
{
    ...
    stepper_location_set = flash_read_word(STEPPER_LOCATION_ADDRESS);
    ...
    if (stepper_location_set > 2000) stepper_location_set = 2000;
