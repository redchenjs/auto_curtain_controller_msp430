...
void DS3231::getTime(byte& year, byte& month, byte& date, byte& DoW, byte& hour, byte& minute, byte& second) {
	...
	DoW = bcdToDec(i2c.read());
	date = bcdToDec(i2c.read());
	month = bcdToDec(i2c.read() & 0b01111111);
...
float DS3231::getTemperature() {
	...
	byte temp;
	i2c.beginTransmission(CLOCK_ADDRESS);
	i2c.write(uint8_t(0x11));
	i2c.endTransmission();

	i2c.requestFrom(CLOCK_ADDRESS, 2);
	temp = i2c.read();	// Here's the MSB
	return float(temp) + 0.25*(i2c.read()>>6);
}
...
void DS3231::getA1Time(byte& A1Day, byte& A1Hour, byte& A1Minute, byte& A1Second, byte& AlarmBits, bool& A1Dy, bool& A1h12, bool& A1PM) {
	...
	temp_buffer	= i2c.read();	// Get A1M1 and A1 Seconds
	...
	temp_buffer		= i2c.read();	// Get A1M2 and A1 minutes
	...
	temp_buffer	= i2c.read();	// Get A1M3 and A1 Hour
	...
	temp_buffer	= i2c.read();	// Get A1M4 and A1 Day/Date
...
void DS3231::getA2Time(byte& A2Day, byte& A2Hour, byte& A2Minute, byte& AlarmBits, bool& A2Dy, bool& A2h12, bool& A2PM) {
	...
	temp_buffer	= i2c.read();	// Get A2M2 and A2 Minutes
	...
	temp_buffer	= i2c.read();	// Get A2M3 and A2 Hour
	...
	temp_buffer	= i2c.read();	// Get A2M4 and A1 Day/Date
...
void DS3231::setA1Time(byte A1Day, byte A1Hour, byte A1Minute, byte A1Second, byte AlarmBits, bool A1Dy, bool A1h12, bool A1PM) {
	...
	i2c.write(decToBcd(A1Second) | ((AlarmBits & 0b00000001) << 7));
	// Send A1 Minute and A1M2
	i2c.write(decToBcd(A1Minute) | ((AlarmBits & 0b00000010) << 6));
	// Figure out A1 hour 
	if (A1h12) {
...
void DS3231::setA2Time(byte A2Day, byte A2Hour, byte A2Minute, byte AlarmBits, bool A2Dy, bool A2h12, bool A2PM) {
	...
	i2c.write(decToBcd(A2Minute) | ((AlarmBits & 0b00010000) << 3));
	// Figure out A2 hour 
	if (A2h12) {
		// Start by converting existing time to h12 if it was given in 24h.
		if (A2Hour > 12) {
